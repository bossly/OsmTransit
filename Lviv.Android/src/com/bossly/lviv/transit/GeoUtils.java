...
public class GeoUtils
{
			...
			for (Route route : data)
			{
				...
				if (route.path != null)
				{
					...
					for (int j = 0, count = points.length; j < count; j++)
					{
						...
						if (j != 0)
						{
							double distance = GeoUtils.distanceToSegment(lat, lng, clat, clng, pclat, pclng);
	...
	public static double distanceToSegment(double x3, double y3, double x1, double y1, double x2,
			double y2)
	{
	...
	public static class Point2D
	{
		...
		public static double distance(double clat1, double clon1, double lat, double lon)
		{
			double R = 6366000; // km
			...
			double t1 = Math.cos(dLat) * Math.cos(dLon) * Math.cos(lat1) * Math.cos(lat2);
			double t2 = Math.cos(dLat) * Math.sin(dLon) * Math.cos(lat1) * Math.sin(lat2);
			...
			long max_min)
	{
		...
		if (currentBestLocation == null)
		{
			...
			long timeDelta = location.getTime() - now.getTime() - now.getTimezoneOffset() * 60000;
